...
export function SettingsPage({ onNavigate }: Props) {
  ...
  // ── MODE TOGGLE — writes to DB via saveAccount, syncs store capital ────
  const handleModeToggle = async (v: boolean) => {
    if (v && !account?.use_real) { setConfirmLiveSwitch(true); return; }
    // Switch to MOCK — no confirmation needed
  ...
  const confirmGoLive = async () => {
    ...
    setToggleSaving(true);
    // Write use_real: true — saveAccount updates local state + DB
    ...
    setToggleSaving(false);
    // Do NOT call refreshBalance here — it would race with the DB write
  };
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 pb-24 md:pb-8">
      ...
      {showPassphrase && user && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/70 px-4">
          <div className="w-full max-w-sm rounded-2xl border border-[#F59E0B]/30 bg-[#0B0E14] p-6 shadow-2xl space-y-4">
            ...
            <p className="text-[11px] text-[#6B7280]">Write down these 12 words. <strong className="text-[#F59E0B]">Only time</strong> shown.</p>
            <div className="grid grid-cols-3 gap-1.5">{passphrase.split(' ').map((w, i) => (<div key={i} className="flex items-center gap-1.5 bg-[#05060B] border border-white/[0.06] rounded-lg px-2 py-1.5"><span className="text-[9px] text-[#374151] w-4">{i+1}.</span><span className="text-xs font-bold text-[#F4F6FA]">{w}</span></div>))}</div>
            <div className="flex gap-2">
      ...
      {msg && <div className={`mb-4 px-4 py-2.5 rounded-xl text-xs font-semibold ${msg.includes('LIVE') || msg.includes('🔴') ? 'bg-red-500/10 border border-red-500/20 text-red-400' : 'bg-[#2BFFF1]/10 border border-[#2BFFF1]/20 text-[#2BFFF1]'}`}>{msg}</div>}
      ...
      <div className="flex gap-1 mb-5 overflow-x-auto pb-1">
        {TABS.map(t => (<button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold whitespace-nowrap flex-shrink-0 ${tab === t.id ? 'bg-[#2BFFF1]/15 text-[#2BFFF1] border border-[#2BFFF1]/25' : 'border border-white/[0.07] text-[#4B5563] hover:text-[#A7B0B7]'}`}><span className="opacity-60" style={{ color: tab === t.id ? '#2BFFF1' : '#4B5563' }}>{Icon[t.icon]()}</span>{t.label}</button>))}
      </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div><label className="text-[10px] text-[#4B5563] mb-1 block font-semibold uppercase">Email</label><input value={user?.email ?? ''} disabled className="w-full bg-[#05060B] border border-white/[0.05] rounded-xl px-4 py-2.5 text-sm text-[#4B5563] cursor-not-allowed"/></div>
              <button onClick={saveProfile} disabled={saving} className="px-5 py-2.5 rounded-xl bg-[#2BFFF1]/15 text-[#2BFFF1] border border-[#2BFFF1]/25 text-sm font-bold disabled:opacity-50">{saving ? 'Saving…' : 'Save Profile'}</button>
            </div>
          </div>
      ...
      {tab === 'trading' && (
        <div className="space-y-4">
          <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5">
            ...
            {account?.use_real && liveSOLUSD === 0 && <div className="rounded-xl bg-[#F59E0B]/08 border border-[#F59E0B]/20 px-3 py-2 mt-1"><p className="text-[10px] text-[#F59E0B]">Live mode active but balance is $0. Deposit funds first.</p></div>}
            {account?.use_real && liveSOLUSD > 0 && <div className="rounded-xl bg-green-500/08 border border-green-500/20 px-3 py-2 mt-1"><p className="text-[10px] text-green-400">🔴 Live · {liveSOL.toFixed(4)} SOL = ${liveSOLUSD.toFixed(2)}</p></div>}
      ...
      {tab === 'wallet' && (
        <div className="space-y-4">
          <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5 space-y-3">
            <div className="flex items-center justify-between"><div><h2 className="text-sm font-bold text-[#F4F6FA]">Platform Wallet</h2><p className="text-xs text-[#4B5563]">Solana deposit address</p></div><span className="text-[9px] px-2 py-0.5 rounded-full bg-green-500/10 text-green-400 border border-green-500/20">Auto-created</span></div>
            {!walletAddr ? (
              <button onClick={loadWallet} disabled={walletLoading} className="w-full py-2.5 rounded-xl border border-[#2BFFF1]/25 text-[#2BFFF1] text-sm font-bold hover:bg-[#2BFFF1]/10 disabled:opacity-50">{walletLoading ? <span className="flex items-center justify-center gap-2"><div className="w-3 h-3 border border-[#2BFFF1]/30 border-t-[#2BFFF1] rounded-full animate-spin"/>Loading…</span> : 'View Wallet Address'}</button>
            ) : (
              <div className="space-y-2"><div className="bg-[#05060B] border border-white/[0.06] rounded-xl p-3"><p className="font-mono text-xs text-[#2BFFF1] break-all">{walletAddr}</p></div><button onClick={() => { navigator.clipboard.writeText(walletAddr); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="w-full py-2 rounded-xl border border-white/[0.08] text-xs font-bold text-[#A7B0B7] hover:text-[#F4F6FA]">{copied ? '✓ Copied' : 'Copy Address'}</button><p className="text-[9px] text-[#374151]">Send SOL to fund your account.</p></div>
            )}
          </div>
          <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5">
            ...
            {[['On-chain SOL', liveSOL > 0 ? `${liveSOL.toFixed(4)} SOL ($${liveSOLUSD.toFixed(2)})` : '$0.00'], ['Funding', `$${(liveSOLUSD > 0 ? liveSOLUSD : (account?.real_balance ?? 0)).toFixed(2)}`], ['Spot (Live)', `$${(account?.spot_live_balance ?? 0).toFixed(2)}`], ['Bots (Live)', `$${(account?.bot_balance ?? 0).toFixed(2)}`], ['Mock', `$${(account?.mock_balance ?? 0).toFixed(2)}`], ['Bots (Mock)', `$${(account?.bot_mock_balance ?? 0).toFixed(2)}`], ['Mode', account?.use_real ? '🔴 Live' : '📌 Mock']].map(([l, v]) => (<Row key={l as string} label={l as string}><span className="text-sm font-mono font-bold text-[#F4F6FA]">{v}</span></Row>))}
          </div>
        </div>
      )}
