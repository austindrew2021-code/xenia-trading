...
import { BotLabPage } from './pages/BotLabPage';
...
function AssetSelector({ current, onChange }: { current: string; onChange: (id: string, addr?: string, pair?: string) => void }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const r = await searchPumpTokens(query);
      setResults(r);
      setSrch(false);
    }, 350);
            ...
            <div className="overflow-y-auto max-h-72">
              {!query && TOP_ASSETS.map(a => (
                <button key={a.id} onClick={() => { onChange(a.id); setOpen(false); }}
                  ...
                  <div className="w-7 h-7 rounded-lg bg-white/[0.05] flex items-center justify-center text-xs font-black text-[#F4F6FA]">
                    {a.label[0].toUpperCase()}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-[#F4F6FA]">{a.label}</p>
                    {(a as any).isPump && <p className="text-[9px] text-[#F59E0B]">Pump.fun</p>}
                  </div>
              ...
              {query && !srch && results.length > 0 && results.map(r => (
                <button key={r.id} onClick={() => { onChange(r.id, r.address, r.pairAddress); setOpen(false); setQuery(''); }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-[#4B5563] truncate">{r.address?.slice(0, 16)}…</p>
                  </div>
                  {r.priceUsd && r.priceUsd > 0 && <span className="text-[10px] text-[#A7B0B7]">${r.priceUsd.toFixed(6)}</span>}
                </button>
              ))}
              {query && !srch && results.length === 0 && (
                <p className="text-center text-xs text-[#4B5563] py-6">No results</p>
              )}
            </div>
          </div>
        </>
...
function getPrecision(p: number): number {
  if (p >= 1) return 4; if (p >= 0.001) return 8; return 10;
}
...
function TradeForm({ livePrice, asset, chartTP, chartSL, onClearChartTPSL }: {
  ...
  const { account, saveAccount, recordTrade, liveSOL, liveSOLUSD } = useAuth();
  ...
  // On-chain SOL is source of truth for live mode
  const liveCap = liveSOLUSD > 0 ? liveSOLUSD : (account?.real_balance ?? 0);
  const cap     = account
    ? (account.use_real ? liveCap : account.mock_balance)
    : capital;

  // Guard: account not yet loaded
  if (!account) return (
  ...
  const executeTrade = async () => {
    ...
    if (pos) {
      ...
      const bal   = account.use_real ? liveCap : account.mock_balance;
  ...
  const levBtnColor = (p: number) => {
    if (levN === p) return p > 100
      ? 'bg-red-500/20 text-red-400 border-red-500/30'
      : p > 50 ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30'
    ...
    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {account.use_real && (
            <span className="text-[9px] font-mono text-[#2BFFF1]/70">
              {liveSOL > 0 ? `${liveSOL.toFixed(4)} SOL` : `$${liveCap.toFixed(2)}`}
            </span>
          )}
      ...
      <div className="mb-3">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <input type="number" value={lev} min="1" max="300"
            ...
            className={`flex-1 bg-[#0B0E14] border rounded-lg px-2 py-1.5 text-xs text-[#F4F6FA] outline-none ${
              isExtrLev ? 'border-red-500/50' : isHighLev ? 'border-yellow-500/40' : 'border-white/[0.08] focus:border-[#2BFFF1]/40'
            }`}/>
          <span className={`text-xs font-black ${isExtrLev ? 'text-red-400' : isHighLev ? 'text-yellow-400' : 'text-[#A7B0B7]'}`}>
            {levN}×
          </span>
        </div>
      ...
      <div className="rounded-xl bg-[#0B0E14] px-3 py-2.5 mb-3 space-y-1.5">
        {([
          ...
          [`Liq ~${(liqPct * 100).toFixed(2)}% away`,
            `$${(side === 'LONG' ? livePrice * (1 - liqPct) : livePrice * (1 + liqPct)).toFixed(6)}`],
          ['Available',
            account.use_real
              ? (liveSOL > 0 ? `${liveSOL.toFixed(4)} SOL ($${liveCap.toFixed(2)})` : `$${liveCap.toFixed(2)}`)
              : `$${cap.toFixed(2)}`,
            sizeN > cap ? '#F87171' : '#4ADE80'],
      ...
      {confirmLive && account.use_real && (
        <div className="rounded-xl border border-[#F59E0B]/30 bg-[#F59E0B]/08 p-3 mb-3 space-y-2">
          ...
          <p className="text-[10px] text-[#A7B0B7]">
            {side} {asset} · <strong className="text-[#F4F6FA]">${sizeN}</strong> × {levN}× = <strong className="text-[#F4F6FA]">${notional.toFixed(0)}</strong> notional
          </p>
          ...
          <div className="flex gap-2">
            <button onClick={() => setConfirmLive(false)}
              className="flex-1 py-2 rounded-xl border border-white/[0.08] text-xs font-bold text-[#A7B0B7] hover:text-[#F4F6FA]">
              Cancel
            </button>
      ...
      <button onClick={submit} disabled={sizeN <= 0 || sizeN > cap || confirmLive}
        ...
        {needsWarn
          ? `⚠️ Confirm ${levN}× — click again to open`
          : account.use_real
            ? `🔴 ${side === 'LONG' ? '▲ Open Long' : '▼ Open Short'} (LIVE)`
    ...
    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4">
      ...
      <div className="space-y-3">
        ...
        <div className="flex justify-between text-[10px]">
          ...
          <span className="font-bold" style={{ color: bull ? '#4ADE80' : '#F87171' }}>{bull ? '▲ Bullish' : '▼ Bearish'}</span>
        </div>
      </div>
    </div>
  );
...
function StatsBar() {
  ...
  // Use on-chain balance as the authoritative capital in live mode
  const cap = account
    ? (account.use_real
        ? (liveSOLUSD > 0 ? liveSOLUSD : account.real_balance)
        : account.mock_balance)
    : capital;

  // Show SOL denomination when live and on-chain balance resolved
  const capLabel = account?.use_real && liveSOL > 0
    ? `${liveSOL.toFixed(4)} SOL`
    : `$${cap.toFixed(2)}`;
  ...
  return (
    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4 h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-1 min-h-0">
        {logs.length === 0
          ? <p className="text-[11px] text-[#4B5563]">No activity yet</p>
    ...
    <button onClick={toggle}
      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-[11px] font-black transition-all ${
        account.use_real
          ...
          : 'border-white/[0.12] bg-white/[0.04] text-[#6B7280] hover:text-[#A7B0B7] hover:border-white/25'
      }`}>
...
const NAV_ICONS: Record<Page, (active: boolean) => React.ReactNode> = {
  ...
  markets:  (a) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#2BFFF1' : '#374151'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>,
  spot:     (a) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#2BFFF1' : '#374151'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>,
  p2p:      (a) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#2BFFF1' : '#374151'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>,
  earn:     (a) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#2BFFF1' : '#374151'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>,
  discover: (a) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#2BFFF1' : '#374151'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>,
  ...
  copy:     (a) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#2BFFF1' : '#374151'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 014-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 01-4 4H3"/></svg>,
  lab:      (a) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#2BFFF1' : '#374151'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2V9M9 21H5a2 2 0 01-2-2V9m0 0h18"/></svg>,
};
        ...
        <div className="flex items-center gap-1 ml-auto flex-shrink-0">
          {INTERVALS.map(i => (
            <button key={i} onClick={() => setInterval(i)}
              className={`px-1.5 py-1 rounded text-[10px] font-bold transition-all ${interval === i ? 'bg-[#2BFFF1]/15 text-[#2BFFF1] border border-[#2BFFF1]/20' : 'text-[#4B5563]'}`}>
              {i}
            </button>
          ))}
          <LiveMockToggle/>
      ...
      <div className="flex border-b border-white/[0.06] flex-shrink-0">
        {([['chart', 'Chart'], ['trade', 'Trade'], ['bots', 'Bots'], ['board', 'Rankings']] as const).map(([t, l]) => (
          <button key={t} onClick={() => setTab(t as any)}
            className={`flex-1 py-2 text-[10px] font-semibold transition-all ${tab === t ? 'text-[#2BFFF1] border-b-2 border-[#2BFFF1]' : 'text-[#4B5563]'}`}>
            {l}
          </button>
        ))}
      </div>
      <div className="flex-1 overflow-hidden">
          ...
          <div className="h-full flex flex-col">
            <div className="flex-1 min-h-0 p-2">
              <PriceChart candles={candles} livePrice={livePrice} positions={positions}
                onPlaceOrder={(_side, tp, sl) => { if (tp != null && onSetTP) onSetTP(tp); if (sl != null && onSetSL) onSetSL(sl); setTab('trade'); }}/>
            </div>
        ...
        {tab === 'trade' && (
          <div className="overflow-y-auto h-full p-3 space-y-3">
            <TradeForm livePrice={livePrice} asset={assetLabel} chartTP={chartTP} chartSL={chartSL} onClearChartTPSL={() => { onSetTP(null); onSetSL(null); }}/>
            <IndicatorsPanel prices={prices}/>
            <BuySellPressure candles={candles} livePrice={livePrice} asset={assetLabel} assetId={assetId}/>
          </div>
        )}
        {tab === 'bots' && (
          <div className="overflow-y-auto h-full p-3 space-y-3">
            <LabBotPanel target="leverage" isMock={true} compact={true}/>
            <BotPanel/>
            <ActivityLog/>
          </div>
        )}
...
export default function App() {
  ...
  // Keep trading store capital in sync with account balance
  useEffect(() => {
    if (account) {
      setCapital(account.use_real
        ? (liveSOLUSD > 0 ? liveSOLUSD : account.real_balance)
        : account.mock_balance);
    }
  }, [account?.use_real, account?.real_balance, account?.mock_balance, liveSOLUSD, setCapital]);
  ...
  const handleChangeAsset = (id: string, addr?: string, pair?: string) => {
    setAssetId(id as AssetId); setCustomAddr(addr); setCustomPair(pair); setPage('trade');
  };
  ...
  // Total display capital in header
  ...
  // Show SOL denomination when on-chain balance resolved
    ...
    <div className="h-screen bg-[#05060B] flex flex-col overflow-hidden" style={{ fontFamily: '-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif' }}>
      ...
      <div className="border-b border-white/[0.06] bg-[#05060B]/90 backdrop-blur-sm sticky top-0 z-50 flex-shrink-0">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          {/* Live price ticker on trade page */}
          {page === 'trade' && (
            ...
            {user && account ? (
              <>
                ...
                <button onClick={() => setShowWallet(true)}
                  className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl border border-white/[0.07] bg-white/[0.02] hover:bg-white/[0.04] transition-all">
                  <div className={`w-2 h-2 rounded-full ${account.use_real ? 'bg-[#2BFFF1] shadow-[0_0_6px_#2BFFF1]' : 'bg-[#374151]'}`}/>
                  <span className="text-[10px] text-[#6B7280] hidden md:block">{user.email?.split('@')[0]}</span>
                  <span className="text-xs font-bold font-mono" style={{ color: account.use_real ? '#2BFFF1' : '#6B7280' }}>
                    {dispCapLabel}
                  </span>
                </button>
                ...
                <button onClick={() => setShowTransfer(true)}
                  className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border border-white/[0.07] text-[#4B5563] hover:text-[#A7B0B7] hover:border-white/20 transition-all text-xs font-semibold">
                ...
                {/* Mobile wallet icon */}
                <button onClick={() => setShowWallet(true)}
                  className="sm:hidden text-xs px-3 py-1.5 rounded-xl border border-[#2BFFF1]/25 text-[#2BFFF1] hover:bg-[#2BFFF1]/10 transition-all">
                  💳
                </button>

                <button onClick={() => signOut()}
                  className="hidden sm:block text-[10px] px-2.5 py-1.5 rounded-xl border border-white/[0.07] text-[#4B5563] hover:text-[#A7B0B7] transition-all">
                  Sign out
                </button>
              </>
            ) : (
              ...
              <div className="flex items-center gap-0.5">
                {INTERVALS.map(i => (
                  <button key={i} onClick={() => setInterval_(i)}
                    className={`px-2 py-1 rounded text-[10px] font-semibold transition-all ${interval === i ? 'bg-[#2BFFF1]/15 text-[#2BFFF1] border border-[#2BFFF1]/20' : 'text-[#4B5563] hover:text-[#A7B0B7]'}`}>
                    {i}
                  </button>
                ))}
              </div>
                ...
                <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-3 flex-1 min-h-0">
                  ...
                  <div style={{ height: 'calc(100% - 28px)' }}>
                    <PriceChart
                      ...
                      onQuickTP={p => setQuickTP(p)} onQuickSL={p => setQuickSL(p)}
                ...
                <div className="flex rounded-xl border border-white/[0.07] overflow-hidden flex-shrink-0">
                  {([['trade', 'Trade'], ['bots', 'Bots'], ['board', 'Rankings']] as const).map(([t, l]) => (
                    <button key={t} onClick={() => setRightTab(t as any)}
                      className={`flex-1 py-2 text-[10px] font-semibold transition-all ${rightTab === t ? 'bg-[#2BFFF1]/15 text-[#2BFFF1]' : 'text-[#4B5563] hover:text-[#A7B0B7]'}`}>
                      {l}
                    </button>
                  ))}
                </div>
